...
class Player {
  constructor(x, y, character) {
    ...
    // --- Charged shot (item-gated) ----------------------------------------
    // Off by default — plain continuous fire like vanilla BoI.
    // The "Charged Shot" item sets hasChargeShot = true to enable it.
    this.hasChargeShot = false;
  ...
  update(dt, room, projectiles) {
    ...
    if (this.hasChargeShot) {
      // --- Charged shot mode (item-unlocked) ---
      const CHARGE_THRESHOLD = 0.55;
      if (arrowHeld) {
      ...
      // --- Normal fire (default) ---
      // Plain continuous fire — hold an arrow and tears come out on cooldown.
      if (arrowHeld && this.fireCooldown <= 0) {
        this.shoot(sx, sy, projectiles);
        this.fireCooldown = this.fireCooldownMs;
        this.lookX = sx; this.lookY = sy;
        this.muzzleFlash = 70;
  ...
  draw(ctx) {
    ...
    // Charge ring — only visible when the Charged Shot item is active.
    // Drawn under the body so the player silhouette stays clean on top.
    if (this.hasChargeShot && this.chargeT > 0.1) {
